---
description: Reads the feature analysis, optionally invokes a design-source adapter for NDS extraction, generates a per-layer technical spec, and writes spec.json validated against the spec schema.
---
# `/aflow-design-feature` — Design Feature Spec
...
2. Read `.pipeline/runs/<ticket_id>/analysis.json`. Verify `type == "feature"`. If missing or wrong type:
   ...
   Run /aflow-analyze-ticket <ticket_id> first (must be a feature ticket).
   ```
   Stop.
...
- **Reuse candidates (graph-verified)** — build/refresh the graph and, for the feature area, list existing nearby components so the spec reuses them instead of re-creating:

  ```bash
  aflow graph --ensure-fresh -o .pipeline/graph.json --project-root .
  aflow graph-query neighbors <area-file-or-symbol> -i .pipeline/graph.json --format json
  ```

  Feed the `outgoing`/`incoming` neighbors into the `widgets_reuse` decision in §4 (presentation): a component already wired near this area is a reuse candidate, not a `widgets_new`. This makes the README's DRY promise verifiable rather than asserted.

  Node `role` (e.g. `riverpod.notifier`, `getx.controller`, `riverpod.consumer_widget`) tells you the project's state-mgmt idiom from real code — design the spec to match it (don't introduce a second state library), and prefer reusing an existing same-role component over a new one. For state dependencies, `aflow graph-query state-flow -i .pipeline/graph.json --format json` shows which providers/controllers nearby widgets already consume — reuse those instead of declaring a parallel one.
...
Validate the assembled object against [`spec.schema.yaml`](../../contracts/schemas/spec.schema.yaml). Stop on validation error — never persist a malformed spec. Write to `.pipeline/runs/<ticket_id>/spec.json` with `approved: false`. The Gate 0 step in `/aflow-pipeline` flips approval after human review.
