# `/validate-functional` — Functional Validation
...
If a criterion cannot be automated (visual-only / multi-device / requires real backend), append it to `validation_report.json::manual_steps[]` with a step-by-step procedure for QA. These do NOT fail the gate; they're surfaced to `/qa-handoff`.
...
[PASSED]: Continue to /create-mr.
[FAILED]: Fix failing tests and re-run /validate-functional.
```
