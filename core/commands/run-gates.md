...
2. Verify `.pipeline/runs/<ticket_id>/` exists. If not:
   ...
   Nothing to gate. Run /analyze-ticket first.
   ```
   Stop.
...
dart run alea analyze \
  ...
  --output-file .pipeline/runs/<ticket_id>/gate_<gate>.json \
  --changed-files <file1> \
  --changed-files <file2> \
  ...
```
...
- **BLOCKER:** try to auto-fix (move imports, invert dependencies, fix layer violations). Re-run the CLI on the affected file. Retry up to 3 times. If still failing:
  ...
  <message>
  Requires manual intervention. Run /run-gates <ticket_id> after fixing.
  ```
...
[FAILED]: Fix the issues above and re-run /run-gates <ticket_id>.
```
