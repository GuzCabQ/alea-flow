# `/implement-presentation` — Implement Presentation Layer
...
```
/implement-presentation DEV-XXXX
```
...
2. Read `.pipeline/runs/<ticket_id>/spec.json`. Verify `approved: true`.
3. Check `spec.design_source.status`:
...
Next: /run-gates <ticket_id> --gate presentation
```
