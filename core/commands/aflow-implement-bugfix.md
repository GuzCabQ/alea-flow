---
description: Applies targeted edits to existing files driven by root_cause_hypothesis and affected_files from analysis.json, then adds regression tests.
---
# `/aflow-implement-bugfix` — Implement Bugfix
...
```
/aflow-implement-bugfix DEV-XXXX
```
...
2. Read `.pipeline/runs/<ticket_id>/analysis.json`. Verify `type: bugfix`. If not:
   ...
   Run /aflow-analyze-ticket first (this command requires type: bugfix).
   ```
   Stop.
...
Next: /aflow-run-gates <ticket_id> --gate bugfix
```
