---
description: Generates the infrastructure layer for a feature — DTOs, repository implementations, and tests — driven by spec.json and the configured code-gen adapter.
---
# `/aflow-implement-infrastructure` — Implement Infrastructure Layer
...
```
/aflow-implement-infrastructure DEV-XXXX
```
...
3. Read `.pipeline/runs/<ticket_id>/spec.json`. Verify `type: feature` AND `approved: true`. If not:
   ...
   ```
   Stop.
4. Verify `domain_impl.md` exists. If not, warn and continue (domain files must exist on disk regardless).
...
Next: /aflow-run-gates <ticket_id> --gate infrastructure  OR  /aflow-implement-presentation <ticket_id>
```
