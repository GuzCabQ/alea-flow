---
description: Generates the domain layer for a feature — pure-Dart entities, value objects, repository contracts, and unit tests — driven by spec.json and the configured code-gen adapter.
---
# `/aflow-implement-domain` — Implement Domain Layer
...
```
/aflow-implement-domain DEV-XXXX
```
...
The code-gen adapter already produces this file. Verify it exists at `<run_dir>/domain_impl.md` and contains the `## files_changed` machine-readable section (one path per line). `/aflow-pipeline` reads this section to stage commits.
...
Next: /aflow-run-gates <ticket_id> --gate domain  OR  /aflow-implement-infrastructure <ticket_id>
```
