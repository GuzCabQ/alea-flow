...
Generates the document QA reads to verify the feature/bugfix manually. Combines automated coverage with the manual steps surfaced by `/validate-functional`.
...
3. Verify the MR was created (check `final_report.json::mr_url` set by `/create-mr`).
...
```
The rollback tag is `pipeline-<ticket_id>-start` (created by `/pipeline` at run start).
