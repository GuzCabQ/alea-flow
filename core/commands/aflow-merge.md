---
description: Documents ALEA's expected merge policy for a feature branch and displays a pre-merge checklist — merging itself remains a human decision.
---
# `/aflow-merge` — Merge Policy Reference
...
- Delete the feature branch locally + on origin:
  ...
  ```
- Run `/aflow-pipeline-feedback <ticket_id>` to capture this run's metrics into `.pipeline/metrics/history.jsonl` (which drives the unreliable-flag circuit breaker in the next run).
...
When merged manually:
  - Run /aflow-pipeline-feedback <ticket_id>
